 ...
 *
 * Models now carry an explicit `namespace:` key (sodaCat 046b9857). Use that
 * when available; otherwise fall back to the family display name (matches
 * the per-vendor convention for ESP/ST/NXP) or the lowercased path segment.
 */
export function deriveNamespace(blockPath, blockData = null) {
  if (blockData && blockData.namespace) return blockData.namespace;
...
export function generatePeripheralHeader(data, blockPath) {
  const ns = deriveNamespace(blockPath, data);
